...
function openAddRecipeModal() {
    ...
    if (modal) {
        ...
        // Populate email from current user session/indicator
        const userEmailIndicator = document.getElementById('userEmail');
        const submittedEmailInput = document.getElementById('submittedEmail');

        if (userEmailIndicator && submittedEmailInput) {
            submittedEmailInput.value = userEmailIndicator.textContent.trim();
        }
        ...
        if (userFullNameSpan && submittedByInput && !submittedByInput.value) {
            submittedByInput.value = userFullNameSpan.textContent.trim();
        }
    }
}
                ...
                .then(data => {
                    if (data.success) {
                        const message = (data.status === 'approved')
                            ? 'Recipe posted successfully! It is now live.'
                            : 'Recipe submitted successfully! It will be reviewed by our team.';

                        alert(message);
                        closeAddRecipeModal();

                        // Reload recipes based on dashboard type
                        if (typeof loadApprovedRecipes === 'function') {
                            loadApprovedRecipes(); // User Dashboard
                        }
                        if (typeof refreshAdminUI === 'function') {
                            refreshAdminUI(); // Full Admin Refresh
                        } else if (typeof loadRecipes === 'function') {
                            loadRecipes(); // Fallback for Admin
                        }
                    } else {
                        alert('Error: ' + (data.message || 'Failed to submit recipe'));
                    }
                })
                .catch(error => {
